...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Vifaa</h4></a></li>
	";
...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Wazalishaji</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>Samahani, umeshachagua kifaa hicho. Tafadhali chagua kifaa kingine!</b>
				</div>
			";
		} else {
			...
			if(mysqli_query($con,$sql)){
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Hongera! Hitaji lako limefanikiwa!</b>
					</div>
				";
			}
		}
	...
	if($count > 0){
		...
		while($row=mysqli_fetch_array($run_query)){
			...
			if(isset($_POST["get_cart_product"])){
				...
				<div class='row'>
					...
					<div class='col-md-3 col-xs-3'>Tsh. $pro_price/=</div>
				</div><br>
			";
			$no = $no + 1;
			}else{
		...
		echo '
				...
				<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
				  ...
				  <input type="hidden" name="business" value="shoppingcart@vifaavyashughuli.com">
...
if(isset($_POST["removeFromCart"])){
	...
	if($run_query){
		echo "
			<div class='alert alert-danger'>
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>Hitaji limeondolewa kwenye idadi ya mahitaji yako, waweza kuendelea kukusanya mahitaji mengine!</b>
			</div>
		";
	}
}
...
if(isset($_POST["updateProduct"])){
	...
	if($run_query){
		echo "
			<div class='alert alert-success'>
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>Hitaji limesahihishwa, waweza kuendelea kukusanya mahitaji mengine!</b>
			</div>
		";
	}
}
